...
/**
 * Classe da janela de redefinição (exclusão e recriação da tabela) dos eventos cadastrados na database.
 * @author Thalles
 *
 */
public class ApagarDatabaseEventos {
	
	public ApagarDatabaseEventos(){
		...
		Label aviso = new Label("Essa ação apagará todos os eventos registrados no banco de dados.");
		...
		try {
			if(OperadorEventos.databaseVazia()==false){
				mainStage.show();
			}else{
				AlertaDeErro alerta = new AlertaDeErro("Apagar database","Database vazia","Não há eventos cadastrados");
			}
		} catch (Exception e1) {
			AlertaDeErro alerta = new AlertaDeErro("Apagar database","Database vazia","Não há eventos cadastrados");
		}
		...
		apagarTudo.setOnAction(new EventHandler(){
			...
			public void handle(Event event) {
				if(senha.getText().equals("1234")){
					try {
						database.Inicializar.apagarDatabaseEventos();
						mainStage.close();
						JanelaPrincipal.atualizar();
						AlertaDeInformacao alerta = new AlertaDeInformacao("Apagar database","Database apagada","Os eventos cadastrados no sistema foram apagados com sucesso");
						Logger.adicionarLog(new Log("REDEFINIÇÃO DE DATABASE: EVENTOS"));
					} catch (Exception e) {
						mainStage.close();
						if(!e.getMessage().contains("EVENTOS")){
					...
					Logger.adicionarLog(new Log("ERRO DE SEGURANÇA - REDEFINIR DATABASE EVENTOS: Senha incorreta"));
				}
