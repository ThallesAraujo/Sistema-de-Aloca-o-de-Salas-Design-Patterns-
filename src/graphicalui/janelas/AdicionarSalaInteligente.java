...
/**
 * Classe da janela de cadastro de salas de aula do tipo inteligente.
 * (Padrão Strategy: classe concreta de estratégia)
 * (Padrão Template Method: implementação do método gabarito)
 * @author Thalles
 *
 */
public class AdicionarSalaInteligente extends AdicionarSala{

	public AdicionarSalaInteligente() {
		super("Sala Inteligente");
	}

	@Override
	public void salvamento() {
		try {
			Invoker.adicionarEExecutar(new ComandoAdicionarSala(new Sala(super.tfCodigo.getText(),super.tfApelido.getText(),Integer.parseInt(super.tfCapacidade.getText()),"Sala de Aula","Inteligente","sim")));
