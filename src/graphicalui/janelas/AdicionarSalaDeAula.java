...
/**
 * Classe da janela de cadastro de salas de aula do tipo normal.
 * (Padrão Strategy: classe concreta de estratégia)
 * (Padrão Template Method: implementação do método gabarito)
 * @author Thalles
 *
 */
public class AdicionarSalaDeAula extends AdicionarSala{

	public AdicionarSalaDeAula() {
		super("Sala de Aula");
	}
	...
	public void salvamento() {
		try {
			Invoker.adicionarEExecutar(new ComandoAdicionarSala(new Sala(super.tfCodigo.getText(),super.tfApelido.getText(),Integer.parseInt(super.tfCapacidade.getText()),"Sala de Aula","Normal","sim")));
